...
public class Test132
{
	public static void main(String[] args)
	{
		...
		//    『e』 문자열이 존재할까? 
		//    존재한다면 그 위치는 어떻게 될까?
		//    (단, 문자열의 끝에서부터 스캔 후 인덱스는 앞에서부터 처리)
		// seoul korea
		System.out.println(s.indexOf("e"));			//--==>> 1
		...
		//System.out.println(s.charAt(20));			//--==>> 런타임 에러 발생(StringIndexOutOfBoundsException)
		...
		s = Integer.toString(30);		// toxxx() 대부분 변화시키는 역할의 메소드			
		System.out.println(s);			
		//--==>> 30
		...
		s = String.format("%d", n);		//-- format() 메소드는 static 메소드 (s.format()으로 쓰지 않음))
		System.out.println(s);
		//--==>> 2345678 
		...
		double p = 3.141592;
		s = String.format("%f", p);		//-- String의 format()는 문자열을 값으로 반환함, s에 문자열 값 넣어줌
		System.out.println(s);			//-- 문자열의 형태로 넘어 온 매개변수를 출력함
		//--==>> 3.141592
		...
		// split() : String 안에 있던 메소드(배열로 값을 반환한다)
		String[] strArray = "기본,열정,배려".split(",");		//-- cf. split("\\s") : 공백 구문
