...
○ Vector(벡터)
   ...
   데이터 요소를 가질 수 있으며, 데이터가 가득 차면 자동으로 젖아 영역을
   늘려주게 된다.

   또한, 객체 레퍼런스를 저장하기 때문에
   JDK 1.5 이전에는 자바 기본형 데이터(primitive type, 프리미티브 자료형)는 저장할 수 없었으나
   ...
   - void addElement(E obj)		//-- 메소드 이름이 길수록 최근에 만들어진 메소드일 확률이 높음
   - boolean add(E e)
     객체를 벡터 끝에 추가한다.
   ...
   - E set(int index, E element)
	 ...
	 주어진 위치에 지정되어 있던 객체는 소멸한다. → 덮어쓰기 개념

   - E elementAt(int index)
   - E get(int index)
     주어진 위치에 저장된 객체를 반환한다.

   - Enumeration<E> elements() //-- Enumeration<E> : 데이터타입, 컬렉션 자료구조를 돕기 위한 또 다른 자료구조 cf.iterator
     벡터의 모든 요소를 반환한다.
...
public class Test153
{
	...
	public static void main(String[] args)
	{
		...
		// get()
		str = (String)v.get(1); //-- 배열의 인덱스와 비슷하므로 '1'을 넘겨줘야 두 번째 요소 출력됨
		...
		// elementAt() → get()과 사용법이 똑같음
		str = (String)v.elementAt(1);
		System.out.println("두 번재 요소 : " + str);
		//--==>> 두 번재 요소 : 노랑 
		...
		while (it.hasNext())		//-- hasNext() : 자체가 true/false 반환
		{
		   str = (String)it.next();
