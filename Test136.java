...
○ 캐릭터 셋(Character Set)
   ...
   - 유니코드(unicode)
	 ...
	 한글이나 일본어는 16비트의 값을 가지게 된느데 이들을 모두
	 16비트로 통일한 것이다.
...
public class Test136
{
	public static void main(String[] args) //throws UnsupportedEncodingException
	{
		...
		//--==>> buf 배열의 length : 6 (6개의 Byte로 구성됨)

		System.out.println();

		// Exception : 예외 처리 구문 (feat. 폭발물 처리반)
		try
		{
			...
			//--==>> value 1 : 남상현
			// new String(buf, "UTF-8")  //-- 매개변수1 : 바이트 배열, 매개변수2 : 인코딩 방식 지정

			System.out.println();
			
			System.out.println("value2 : " + new String(buf));
			//--==>> value2 : ?궓?긽?쁽
			// new String(buf) //-- 매개변수1 : 바이트 배열, (매개변수2 : 생략 -- 인코딩 방식 'EUC-KR'로 자동설정됨)
			...
			buf = convert.getBytes("UTF-8");	// UTF-8
			System.out.println("buf 배열의 Length : " + buf.length);
