...
○ 객체 직렬화(Object Serialization)는
   ...
   따라서, 메소드와 생성자는 객체 직렬화의 대상에서 제외된다.(정적 변수, 클래스 변수, static 변수 포함)
...
○ 객체 직렬화의 장점
   ...
   ※ Bean(빈)
      Client Server 구조적 모델에서 서버측 구조에 해당하며
	  재사용 가능한 소프투웨어 개체를 만들 수 있게 하는 컴포넌트(독립적인 단위 모듈) 기술.
...
public class Test171
{
	public static void main(String[] args) throws Exception
	{
		...
		//-- 파일 전용 출력 스트림(물줄기) 객체에 f0 라는 파일 객체를 띄우겠다... //(정수되는 물)
		
		// 객체 전용 출력 스트림 생성
		ObjectOutputStream oos = new ObjectOutputStream(fos);
		//-- 객체 전용 출력 스트림(물줄기) 객체로
		//   fos 라는 파일 전용 출력 스트림을 감싸겠다... //(정수기 필터)
		...
		if (f0.exists())	// f0 파일 객체가 존재한다면...
		{
			...
			// ※ Hashtable은 Iterator 사용할 수 없음 check~!!!
			Enumeration e = h2.keys();		//-- Enumeration 형태로 반환됨
